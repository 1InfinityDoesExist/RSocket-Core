...
class RSocketServerApplicationTests {
	...
	public void testRequestGetsResponse() {
		...
		Mono<Message> response = requester.route("request-response").data(new Message("TEST"))
	...
	public void testRequestStream() {
		// Send a request message
		Flux<Message> stream = requester.route("request-stream").data(new Message("TEST")).retrieveFlux(Message.class);
		...
		}).expectNextCount(0).consumeNextWith(message -> {
	...
	public void testStreamGetsStream() {
		...
		Flux<Message> stream = requester.route("stream-stream").data(settings).retrieveFlux(Message.class);
